#!/usr/bin/env bash
# Aave V3에서 USDe 0.5를 대출합니다.
# 사전 조건: USDC 1.0 이상 supply 완료
...
HEALTH=$(echo "$POSITIONS" | jq -r '.account.health_factor // "no_debt"')
echo "total_collateral_usd: $COLLATERAL"
echo "health_factor: $HEALTH"

if (( $(echo "$COLLATERAL < 0.5" | awk '{print ($1 < $3)}') )); then
  echo "오류: 담보가 부족합니다. aave-supply-usdc/supply.sh 를 먼저 실행하세요."
  exit 1
fi

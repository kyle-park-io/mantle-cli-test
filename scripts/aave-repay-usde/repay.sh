#!/usr/bin/env bash
# Aave V3 USDe 0.5 대출을 상환합니다.
# 순서: 1) approve USDe → Aave Pool → 2) repay
...
# ---------------------------------------------------------------------------
# Step 1: approve USDe → Aave Pool (nonce N)
# ---------------------------------------------------------------------------

echo ""
echo "[1/2] approve USDe → Aave Pool 중... (nonce=$NONCE)"

yarn --cwd "$ROOT_DIR" mantle-cli swap approve \
  --token USDE --spender "$AAVE_POOL" --amount "$AMOUNT" --owner "$WALLET" \
  --json > "$APPROVE_FILE"

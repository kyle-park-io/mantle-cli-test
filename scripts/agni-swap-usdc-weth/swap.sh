#!/usr/bin/env bash
# Agni에서 USDC 10을 WETH로 스왑합니다.
#
# 목적: Aave V3 Mantle에서 담보로 사용하기 위한 WETH 확보
# 배경: Mantle Aave V3에서 담보(LTV > 0)로 쓸 수 있는 자산은 WETH(LTV 80.5%)와
#       WMNT(LTV 40%, borrow 불가)뿐입니다. USDC는 LTV=0이라 담보 불가.
...
# ---------------------------------------------------------------------------
# Step 1: swap-quote → minimum_out_raw / fee_tier 추출
# ---------------------------------------------------------------------------

echo ""
echo "[1/3] swap-quote 조회 중... (USDC $AMOUNT → WETH, $PROVIDER)"

QUOTE=$(yarn --cwd "$ROOT_DIR" mantle-cli defi swap-quote \
  --in USDC --out WETH --amount "$AMOUNT" --provider "$PROVIDER" --json 2>/dev/null)
...
echo ""
echo "[2/3] approve USDC → router 중... (nonce=$NONCE)"

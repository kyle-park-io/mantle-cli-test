#!/usr/bin/env bash
# Merchant Moe에서 USDC 0.1을 USDe로 스왑합니다.
...
# ---------------------------------------------------------------------------
# Step 1: swap-quote → minimum_out_raw / router / bin_step 추출
# ---------------------------------------------------------------------------
...
BIN_STEP=$(echo "$QUOTE" | jq -r '.resolved_pool_params.bin_step')
echo "router:          $ROUTER"
echo "minimum_out_raw: $MIN_OUT"
echo "bin_step:        $BIN_STEP"
